...
/**
 * Created by zhangwenjun on 2017/9/24.
 */
...
//        byte[] pixels = new byte[(rgba.length / 4) * 4];
//
//        int count = rgba.length / 4;
//
//        //Bitmap像素点的色彩通道排列顺序是RGBA
...
//        return pixels;
    ...
    public static Bitmap rgb2Bitmap(byte[] imageData,int width,int height){
        int[] argb = new int[width*height];
        for (int i=0; i<height; i++) {
            for (int j=0; j<width; j++) {
                argb[i * width + j] = 255;
    ...
    public static Bitmap yuv2Bitmap(byte[] imageData, int width, int height, Rect rect) {
        try {
